...
class CaptchaBot(SingleServerIRCBot):

    def __init__(self, server_list, nickname, chanlist, **connect_params):
        super(CaptchaBotBot, self).__init__(
            server_list, nickname, "CaptchaBot Bot", **connect_params)
        self.chanlist = chanlist
        self.db = CaptchaBotDB(
                settings["db_host"],
                settings["db_user"],
                settings["db_pass"],
                settings["db_name"])
    ...
    def hook_control_msg(self, c, e, argv, reply):
        ...
        if cmd == "seclevel":
            ...
            if seclevel in [ "OFF", "SOFT", "SECURE" ]:
                ...
                c.privmsg(reply, "CaptchaBot level set to %s" % seclevel)
            else:
